...
Hello! Today we learn about Exploratory Data Analysis in R using tidyverse functions.
...
### Visualise the selling price distribution
...
```{r skim, exercise=TRUE, exercise.lines = 4}
____(ameshousing)
```
...
ameshousing %>% 
  ...
  labs(____ = "Sale Price in $k's",
       ____ = "Number of Houses sold")
```

```{r selling-price-solution}
ameshousing %>% 
...
ameshousingCor <- cor(ameshousing[,____],
...
-   Create a data split that divides the original data into 80%/20% sections and (roughly) divides the data into subsets based on the target/outcome variable's distribution;
-   Assign the 80% partition to `ames_train` and the 20% partition to `ames_test`.
...
#ames_rec %>% prep() %>% tidy(number = 1) to get the picked transformation
```
...
**Instructions** Create a workflow, start with `workflow()` to create an empty workflow and then add out model and recipe with `add_model()` and `add_recipe()`:

```{r wf, exercise=TRUE, exercise.blanks = "___+"}
ames_train <- qread(here("inst/tutorials/Part1/models", "ames_train.qs"))
...
question(
  ...
  answer("randomly divide your training data into 10 subsets and train on 9 at a time (assessing on the other subset), iterating through all 10 subsets for assessment. Then you repeat that process 5 times"),
  answer("randomly divide your training data into 5 subsets and train on 4 at a time (assessing on the other subset), iterating through all 5 subsets. Then you repeat that process 10 times", correct = TRUE),
  allow_retry = TRUE,
...
qsave(ames_folds, here("inst/tutorials/Part1/models", "ames_folds.qs"))
```
...
*Warnings: Linear regression detects some redundancies in the predictor set. We can ignore the warnings since lm() can deal with it.*

```{r train, exercise=TRUE, exercise.blanks = "___+"}
ames_train <- qread(here("inst/tutorials/Part1/models", "ames_train_10_perc.qs"))
...
```{r train-solution}
ames_train <- qread(here("inst/tutorials/Part1/models", "ames_train.qs"))
ames_folds <- qread(here("inst/tutorials/Part1/models", "ames_folds.qs"))
...
qsave(ames_res, here("inst/tutorials/Part1/models", "ames_res.qs"))
```
...
ames_res <- qread(here("inst/tutorials/Part1/models", "ames_res.qs"))
...
```{r test, exercise=TRUE, exercise.blanks = "___+"}
ames_split <- qread(here("inst/tutorials/Part1/models", "ames_split.qs"))
ames_train <- qread(here("inst/tutorials/Part1/models", "ames_train.qs"))
...
qsave(ames_final, here("inst/tutorials/Part1/models", "ames_final.qs"))
```

The rsq and rmse metrics are similar for both the training and testing datasets in our linear regression model. This is a good sign that the model is not over-fitting and can be used for making predictions on new data.
...
ames_final <- qread(here("inst/tutorials/Part1/models", "ames_final.qs"))

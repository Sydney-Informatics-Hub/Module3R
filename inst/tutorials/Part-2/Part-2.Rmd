---
title: "Getting Started with Machine Learning in R - Part 2"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: false
...
Today, we are going to be working with [Pima Indian Women's diabetes
...
-   Use the `vis_dat()` function to visualise the missing values of the
...
individual cases so that both sets are balanced in selling price.
...
-   Use `bind_rows()` to bind together the rows of the training and
    testing sets;
-   Use `gglot()` to plot a histogram of the output (`diabetes`) to
    check we stratified appropriately.

```{r strata, exercise=TRUE, exercise.blanks = "___+"}
...
**Instructions** Use the `recipe()` function to apply the following
preprocessing steps:
...
diabetes_rec <- ____(diabetes ~ ., data = d_na_train) %>%
                ____(all_predictors()) %>% # perform median imputation
...
**cross-validation**. So we can create a cross-validated version of the
...
```{r update, exercise=TRUE, exercise.blanks = "___+"}
set.seed(123)
...
#ROC curve
  ...
  ____(truth  = diabetes, event_level="second", estimate = .pred_pos) %>%  #specify which level of truth to consider as the "event"
                autoplot()
...
```{r roc-solution}
  collect_predictions(final_diabetes_fit) %>%
  roc_curve(truth  = diabetes, event_level="second", estimate = .pred_pos) %>%
                autoplot()
...
```{r vip, exercise=TRUE, exercise.blanks = "___+"}
